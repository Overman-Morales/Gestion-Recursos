  ...
  ftddata:  DatosFTD [] = [];
    ...
    delete(ftddata: DatosFTD): void {
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.isConfirmed) {
            this.serviceFTD.deletedDatosFTD(ftddata).subscribe((response) => {
              swalWithBootstrapButtons.fire({
                title: '¡Eliminado!',
              ...
              // Redirige al usuario a la página de datos FTD dados de baja
              this.router.navigate(['/datos-baja-ftd']);

              // Actualiza la lista de datos FTD
              this.getDatosFTD();
            });
